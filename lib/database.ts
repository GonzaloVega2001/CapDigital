...
// Función para obtener lecciones de un curso desde la base de datos
...
// Función para obtener una lección específica
...
export async function completeLessonByCourse(userId: string, courseId: number, localLessonId: number) {
  try {
    ...
    return { progress, error: null, success: true }
  } catch (error) {
    return { progress: null, error, success: false }
  }
}
...
// Función alternativa más robusta para obtener progreso del curso
export async function getCourseProgressRobust(userId: string, courseId: number) {
  try {
    ...
    if (lessonsError) {
      return { progress: 0, error: lessonsError }
    }

    if (!allLessons || allLessons.length === 0) {
      return { progress: 0, error: "No lessons found for this course" }
    }

    // Obtener progreso del usuario para estas lecciones
...
export async function forceUpdateProgress(userId: string) {
  try {
    // Obtener todo el progreso actual
    const { data: progress, error } = await supabase
      .from("user_progress")
      .select(`
        lesson_id,
    ...
    // Recalcular progreso por curso
    const progressByCourse: { [key: number]: number } = {}
    
    for (let courseId = 1; courseId <= 6; courseId++) {
      const { progress: courseProgress } = await getCourseProgress(userId, courseId)
